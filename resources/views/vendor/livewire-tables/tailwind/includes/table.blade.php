<x-livewire-tables::table>
        ...
        @foreach ($columns as $column)
            @if ($column->isVisible())
                @if ($column->isBlank())
                    <x-livewire-tables::table.heading />
                @else
                    <x-livewire-tables::table.heading :sortable="$column->isSortable()" :column="$column->column()"
    ...
    <x-slot name="body">
        @forelse ($rows as $index => $row)
            <x-livewire-tables::table.row wire:loading.class.delay="opacity-50"
                wire:key="table-row-{{ $row->getKey() }}"
                :url="method_exists($this, 'getTableRowUrl') ? $this->getTableRowUrl($row) : ''" :class="
                ($index % 2 === 0 ?
                'bg-white dark:bg-gray-800' . (method_exists($this, 'getTableRowUrl') ? ' hover:bg-gray-100' : '') :
                    ...
                    <x-livewire-tables::table.cell>
                        <div class="inline-flex rounded-md shadow-sm">
                            <input wire:model="selected" wire:loading.attr.delay="disabled"
                                value="{{ $row->{$primaryKey} }}" onclick="event.stopPropagation();return true;"
                                type="checkbox"
                                class="rounded-md shadow-sm border-gray-300 block transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                        </div>
                    </x-livewire-tables::table.cell>
