<div x-data="{datepickerValue: @entangle($attributes->wire('model')), ...app()}" x-init="[initDate(), getNoOfDays()]" x-cloak>
    <label for="datepicker" class="text-gray-600 text-xs font-semibold px-1">Tanggal Lahir</label>
    <div class="relative">
        <input type="hidden" name="{{$name}}" x-ref="date" :value="datepickerValue" />
        <input type="text" x-on:click="showDatepicker = !showDatepicker" x-model="datepickerValue" x-on:keydown.escape="showDatepicker = false" class="w-full pl-11 pr-10 py-3 text-gray-700 leading-none rounded-lg shadow-sm focus:outline-none font-medium border-2 border-gray-200 focus:border-yellow-500" placeholder="Select date" readonly />
        <div class="absolute top-1 left-0 px-3 py-2">
