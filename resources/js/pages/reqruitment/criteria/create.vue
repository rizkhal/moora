        ...
        <div class="flex flex-col space-y-4 p-4 border-dashed border mt-2 rounded-md">
          <div class="flex-none">
            <label class="inline-flex items-center">
              <input v-model="form.allow_file_upload" type="checkbox" class="form-checkbox h-4 w-4 text-red-600 border-gray-400 focus:outline-none focus:ring-0 text-sm" />
          ...
          <div v-for="(option, index) in form.options" :key="index" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex-none">
              <button @click.prevent="remove(index)" :disabled="form.options.length === 1" class="disabled:opacity-75 disabled:cursor-not-allowed btn-red btn-ring md:w-12 w-full md:mt-6 py-[11px]">
  ...
  methods: {
    store() {
      this.form
        .transform((data) => ({
          ...data,
          reqruitment_id: this.reqruitment.id
        }))
        .post(`/reqruitment/${this.reqruitment.id}/criteria`, {
        onSuccess: (success) => {
          this.form.reset();
          this.$toast.success('Berhasil menambah kriteria');
        },
      });
    },
    clone() {
